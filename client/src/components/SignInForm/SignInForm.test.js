...
describe('testing SignInForm component', () => {
    ...
    it('submits the form and displays success message on successful login', async () => {
        ...
        fireEvent.change(getByPlaceholderText('login or email'), { target: { value: 'testuser@example.com' } });
        fireEvent.change(getByPlaceholderText('password'), { target: { value: 'testPassword123' } });

        fireEvent.click(getByText('Send'));

        await waitFor(() => {
            expect(axios.post).toHaveBeenCalledWith("http://localhost:4000/api/customers/login", {
