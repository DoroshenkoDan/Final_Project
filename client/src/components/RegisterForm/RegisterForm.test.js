...
describe('testing RegisterForm component', () => {
    ...
    it('submits the form', async () => {
        ...
        fireEvent.change(getByPlaceholderText('lastName'), { target: { value: 'DoeDoe' } });
        fireEvent.change(getByPlaceholderText('login'), { target: { value: 'johndoe123' } });
        ...
        fireEvent.change(getByPlaceholderText('telephone'), { target: { value: '+380501234567' } });

        fireEvent.click(getByText('Send'));

        await waitFor(() => {
            expect(axios.post).toHaveBeenCalledWith("http://localhost:4000/api/customers", {
                firstName: 'John',
                lastName: 'DoeDoe',
                login: 'johndoe123',
