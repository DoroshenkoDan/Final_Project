...
describe('CartProductList', () => {
  ...
  it('shortens the description if it is longer than 50 characters', () => {
    const longDescription = 'A chair made for those lovely dinners with family and friends....';
    const propsWithLongDescription = { ...mockProps, discribe: longDescription };

    const { getByText } = render(<CartProductList {...propsWithLongDescription} />);
    
    expect(getByText('A chair made for those lovely dinners with family ...')).toBeInTheDocument();
  });
});
